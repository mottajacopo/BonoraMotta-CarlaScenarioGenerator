...
class KeyboardControl(object):
    ...
    def parse_events(self, client, world, clock):
        ...
        if(self._lights == carla.libcarla.VehicleLightState.NONE):
            current_lights ^= carla.VehicleLightState.HighBeam
...
def save_image(image, savePath, imageName):
    imageName= str(imageName) + '.jpg'
    finalPath = savePath + imageName
    pygame.image.save(image, finalPath )
...
def save_video(image, savePath, videoName):
    ...
    finalPath = savePath + videoName

    if (firstPass):

        out = cv2.VideoWriter(finalPath, cv2.VideoWriter_fourcc(*'DIVX'), 20, frameSize)
        firstPass=False
 
    view = pygame.surfarray.array3d(image)  # create a copy of the surface
